...
Si vamos a generar documentos en formato `pdf` necesitamos instalar también una distribución de `TeX`. En Windows, se puede instalar [`MiKTeX`](https://miktex.org/download), en macOS se puede instalar [`MacTeX`](http://www.tug.org/mactex/), y en Linux se puede instalar [`TeX Live`](https://www.tug.org/texlive/), aunque el método más sencillo es instala la distribución `TinyTex`, que es una versión reducida de `TeX Live`, mediante el comando
...
si queremos añadir `TinyTex` al `PATH` del sistema para poder utilizarlo desde la línea de comandos.
...
Los bloques de código en Python se renderizan por defecto con el kernel `python3` de `jupyter`. Conviene además tener instalado el paquete `jupyter-cache` para que los resultados de la ejecución de los bloques de código se guarden en caché y no se ejecuten de nuevo si no han cambiado.
...
Por último, también se puede especificar el formato de la bibliografía en el documento final con la propiedad `cls: estilo`, donde `estilo` es el estilo de la bibliografía en formato [CSL](https://citationstyles.org/) (citation style language). Por defecto, se utiliza el estilo [Chicago](https://www.chicagomanualofstyle.org/home.html) pero pueden utilizarse otros estilos como los que aparecen en el [repositorio de estilos CSL](https://github.com/citation-style-language/styles).
...
Quarto permite personalizar la apariencia del documento final mediante temas, especialmente cuando el documento de salida de HTML. Un tema es un conjunto de estilos que se aplican al documento final para cambiar la tipografía, los colores, los márgenes, el espaciado, etc. Quarto incluye varios [temas predefinidos](https://quarto.org/docs/output-formats/html-themes.html), pero también es posible crear temas personalizados mediante hojas de estilo en cascada (CSS).
