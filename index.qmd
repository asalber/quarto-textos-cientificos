...
Quarto puede descargarse desde el [sitio web de Quart](https://quarto.org/docs/get-started/). Existen versiones para Windows, macOS y Linux, así como una versión basada en Docker para aquellos que prefieren trabajar con contenedores.
...
1.  **Creación del documento**. El primer paso es la creación de un fichero de texto plano con extensión `.qmd` (Quarto Markdown). Este fichero normalmente consta de la siguientes partes:
    ...
    -   **Bibliografía**. Se escribe en formato [BibTeX](https://es.wikipedia.org/wiki/BibTeX) y en ella se incluyen las ref erencias bibliográficas que se citan en el cuerpo del documento.
